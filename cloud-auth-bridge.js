...
const SUPABASE_KEY='sb_publishable____2MhGKdMY2FWa_lgOT_w_454NS9Qg';
...
function markProfileComplete(id,complete,name){const list=accounts(),i=list.findIndex(a=>a.id===id);if(i<0)return;if(complete)list[i].profileComplete=true;if(name)list[i].name=name;setJSON(ACCOUNTS_KEY,list)}
function authHeaders(token){return {'Content-Type':'application/json','apikey':SUPABASE_KEY,'Authorization':'Bearer '+token}}
async function reconcileData(a,s){const user=s?.user||cloudStored()?.user,token=s?.access_token||cloudStored()?.access_token;if(!user?.id||!token)return;const r=await fetch(`${SUPABASE_URL}/rest/v1/user_tracker_data?user_id=eq.${encodeURIComponent(user.id)}&select=data,updated_at`,{headers:authHeaders(token)});if(!r.ok)throw new Error('Could not load your cloud tracker data.');const rows=await r.json(),cloud=rows[0]?.data;const localRaw=localStorage.getItem(userDataKey(a.id))||localStorage.getItem(DATA_KEY),local=localRaw?JSON.parse(localRaw):null;if(cloud&&Object.keys(cloud).length){const raw=JSON.stringify(cloud);nativeSet.call(localStorage,DATA_KEY,raw);nativeSet.call(localStorage,userDataKey(a.id),raw);markProfileComplete(a.id,Boolean(cloud.profile),cloud.profile?.name||'');return}if(local&&Object.keys(local).length){await fetch(`${SUPABASE_URL}/rest/v1/user_tracker_data?on_conflict=user_id`,{method:'POST',headers:{...authHeaders(token),'Prefer':'resolution=merge-duplicates'},body:JSON.stringify({user_id:user.id,data:local,updated_at:new Date().toISOString()})})}}
async function establishCloudUser(s,email){const user=s?.user||cloudStored()?.user;if(!user?.id)throw new Error('Could not establish the cloud session. Please log in again.');const existing=accounts().find(a=>normalize(a.email)===normalize(email));const a=createLocalMapping(email,user.id);nativeSet.call(localStorage,SESSION_KEY,a.id);const saved=localStorage.getItem(userDataKey(a.id));if(saved)nativeSet.call(localStorage,DATA_KEY,saved);else if(!existing)nativeRemove.call(localStorage,DATA_KEY);stripLegacyPassword(email,user.id);await reconcileData(a,s);window.location.reload()}
