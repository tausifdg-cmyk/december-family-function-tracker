...
function showLogin(host,error){
  ...
  host.innerHTML=`<div class="admin-card"><h3>Admin Login</h3><p>${esc(error||'Sign in to MYBODY Cloud to open Users Report.')}</p><label style="display:block;margin-top:14px">Email<input id="adminCloudEmail" type="email" value="${ADMIN_EMAIL}" readonly style="width:100%;box-sizing:border-box;margin-top:7px;min-height:50px;border-radius:12px;border:1px solid #384038;background:#090c0a;color:#fff;padding:12px"></label><label style="display:block;margin-top:12px">Password<input id="adminCloudPass" type="password" autocomplete="current-password" style="width:100%;box-sizing:border-box;margin-top:7px;min-height:50px;border-radius:12px;border:1px solid #384038;background:#090c0a;color:#fff;padding:12px"></label><button id="adminCloudLogin" class="primary" type="button" style="width:100%;margin-top:14px">Login & Open Report</button><p id="adminCloudMsg" style="min-height:22px;margin-top:10px"></p></div>`;
  const pass=host.querySelector('#adminCloudPass'),login=host.querySelector('#adminCloudLogin'),msg=host.querySelector('#adminCloudMsg');
