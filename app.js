...
function render(){
 ...
 const week=window.DecemberPlan?DecemberPlan.getCurrentPlanWeek():Math.max(1,Math.min(12,Math.ceil((new Date()-planStart())/604800000)));
 const target=window.DecemberPlan?DecemberPlan.buildWeeklyPlan()[week-1]:null;
 $('#weekLabel').textContent=target?`Week ${week} • ${target.targetWeightKg.toFixed(1)} kg target`:`Week ${week}`;$('#weekProgress').style.width=`${Math.round(week/12*100)}%`;
...
function planStart(){const d=new Date(GOAL_DATE);d.setDate(d.getDate()-(12*7-1));return d}
