(function () {
  ...
  function renderPlannerStep(modal, data) {
    ...
    if (step === 1) {
      ...
      body.innerHTML = `${plannerProgress(step)}<div class="coach-step-copy"><h3>Food preferences</h3><p>Nutrition should fit your culture, routine and preferences instead of forcing a rigid meal plan.</p></div><div class="coach-form-grid"><label>Diet<select name="diet">${selectOptions([['non_vegetarian','Non-vegetarian'],['vegetarian','Vegetarian'],['eggetarian','Eggetarian'],['vegan','Vegan'],['proteinaholic','Proteinaholic whole-food approach'],['no_preference','No preference']],data.diet)}</select></label><label>Cuisine<select name="cuisine">${selectOptions([['indian_mixed','Indian / mixed'],['international','International'],['no_preference','No preference']],data.cuisine)}</select></label><label class="wide">Allergies / intolerances <span>optional</span><input name="allergies" placeholder="e.g. lactose, peanuts, gluten" value="${esc(data.allergies)}"></label><label class="wide">Foods you avoid or dislike <span>optional</span><input name="dislikes" placeholder="e.g. fish, mushrooms" value="${esc(data.dislikes)}"></label></div>${plannerActions(true)}`;
    } else {
      const conditions = [
