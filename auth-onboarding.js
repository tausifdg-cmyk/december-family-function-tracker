...
(function(){
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='authStyles';s.textContent=`
      .auth-gate{position:fixed;inset:0;z-index:99999;background:linear-gradient(160deg,#07101f,#10192c 55%,#07101f);overflow:auto;padding:calc(24px + env(safe-area-inset-top)) 18px 40px;color:#f6f8ff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.auth-shell{max-width:680px;margin:0 auto}.auth-brand{padding:18px 4px 24px}.auth-brand b{font-size:26px}.auth-brand p{color:#9ca8bd;margin:7px 0 0}.auth-card{background:#111b2d;border:1px solid #26334a;border-radius:24px;padding:20px;box-shadow:0 20px 60px #0005}.auth-tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:18px}.auth-tabs button{padding:12px;border-radius:14px;border:1px solid #2a3953;background:#0b1424;color:#aeb8ca;font-weight:700}.auth-tabs button.active{background:#f7f8fb;color:#111}.auth-form{display:grid;gap:13px}.auth-form label{font-size:13px;color:#aeb8ca}.auth-form input,.auth-form select{width:100%;box-sizing:border-box;margin-top:6px;border:1px solid #34425a;background:#0a1322;color:#fff;border-radius:13px;padding:13px;font-size:16px}.auth-btn{border:0;border-radius:14px;padding:14px;font-weight:800;font-size:15px;background:#fff;color:#111}.auth-secondary{background:#18253a;color:#fff;border:1px solid #34425a}.auth-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.auth-note{font-size:12px;color:#91a0b7;line-height:1.45}.auth-error{color:#ff8e8e;font-size:13px;min-height:18px}.auth-step{color:#7d91ff;font-weight:800;font-size:12px;letter-spacing:.12em}.plan-preview{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin:14px 0}.plan-preview div{background:#0a1322;border:1px solid #2c3950;border-radius:15px;padding:13px}.plan-preview small{color:#91a0b7}.plan-preview b{display:block;font-size:20px;margin-top:4px}.user-chip{display:flex;gap:8px;align-items:center}.user-chip button{border:1px solid var(--line);background:var(--card);color:inherit;border-radius:12px;padding:8px 10px}.profile-banner{margin-bottom:14px}.profile-banner strong{display:block}.profile-banner small{color:var(--muted)}@media(max-width:560px){.auth-grid,.plan-preview{grid-template-columns:1fr}}
  ...
  function showLogin(mode='login'){
    ...
    g.innerHTML=`<div class="auth-shell"><div class="auth-brand"><b>Transformation Tracker</b><p>Your food, workouts, measurements and plan in one place.</p></div><div class="auth-card"><div class="auth-tabs"><button id="loginTab" class="${mode==='login'?'active':''}">Log in</button><button id="signupTab" class="${mode==='signup'?'active':''}">Create account</button></div><form id="authForm" class="auth-form"><label>Email<input id="authEmail" type="email" autocomplete="email" required></label><label>Password<input id="authPass" type="password" minlength="6" autocomplete="current-password" required></label><div id="authError" class="auth-error"></div><button class="auth-btn" type="submit">${mode==='login'?'Log in':'Create account'}</button></form><p class="auth-note">Accounts in this version are stored separately on this device/browser. Cloud login and cross-device sync require connecting a backend such as Supabase or Firebase.</p></div></div>`;
    document.getElementById('loginTab').onclick=()=>showLogin('login');document.getElementById('signupTab').onclick=()=>showLogin('signup');
